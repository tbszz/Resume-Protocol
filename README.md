...
1. 统一 Agent 入口
   - 首页提供一个自然语言入口，支持“诊断简历、匹配岗位、生成岗位版、准备面试”四类任务。
   - Agent 会读取职业资料、目标 JD、岗位版和面试计划的真实就绪状态，再决定直接执行或先补齐前置步骤。
   - 当前采用确定性的本地意图路由，不依赖视频加载，也不把静态快捷按钮伪装成通用大模型对话。
   - 视频背景加载失败时自动降级为深墨绿色背景，完整工作流保持可用。
